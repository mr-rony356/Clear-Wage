...
import { useMediaQuery, useTheme, CircularProgress, TextField, Select, MenuItem } from '@mui/material';
...
const Results = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://script.google.com/macros/s/AKfycbxWJzD7pTYLpFvgi_Y7PuLhKCpbUAW9FR_TLupd-HoIkJVqcknqw7KRfrWf3O5XxsU/exec');
  ...
  return (
    <div id='last' style={{ position: 'relative', display: 'flex', flexDirection: 'column', gap: '20px', alignItems: 'center', textAlign: 'center', margin: isMobile ? '20px 5px' : '50px ' }}>
      ...
      <div style={{ width: '100%', height: '25px', textAlign: 'right' }}>
        ...
        >
          ...
          <MenuItem value="FirmName">Firm Name</MenuItem>
      ...
      <ul style={{ listStyleType: 'none', padding: 0, border: '1px solid black', width: isMobile ? '100%' : '100%', marginTop: isMobile ? '10px' : '0' }}>
        <li style={{
          ...
          <span style={{ flex: 2 }}>Firm Name</span>
          ...
          <span style={{ flex: 2 }}> Ethnicity</span>
        </li>
        {loading ? (
          ...
          sortedData.map((rowData, index) => (
            <li key={index} style={{
              ...
              <span style={{ flex: 2 }}>{rowData.FirmName}</span>
              <span style={{ flex: 2 }}>${rowData.Salary}</span>
              <span style={{ flex: 2 }}>${rowData.Bonuses}</span>
              <span style={{ flex: 2 }}>{rowData.City}</span>
              <span style={{ flex: 2 }}>{rowData.State}</span>
              <span style={{ flex: 2 }}>{rowData.Gender}</span>
              <span style={{ flex: 2 }}>{rowData.Ethnicity}</span>
            </li>
          ))
        )}
        {!loading && sortedData.length === 0 &&
          <li style={{ padding: '10px 0', color: 'black', fontSize: '14px' }}> No Data found</li>
        }
      </ul>
    </div >
  );
